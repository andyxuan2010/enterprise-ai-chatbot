...
variable "features" {
  description = "Optional high-level feature switches. Known keys include enable_app_service_stack, enable_app_registration_for_appservice, enable_openai, enable_azure_ai_service, enable_azure_ai_search, and create_azure_search_query_key. Unspecified keys fall back to the existing individual variables."
  type        = map(bool)
  default     = {}
}
...
variable "app_service_custom_hostname_bindings" {
  description = "Advanced custom hostnames to bind to the App Service. For a single hostname, prefer app_service_custom_hostname or TF_VAR_app_service_custom_hostname. Create public DNS CNAME records to app_service_default_hostname before enabling a binding."
  ...
  validation {
    condition = alltrue([
      for binding in values(var.app_service_custom_hostname_bindings) :
      contains(["SniEnabled", "IpBasedEnabled", "Disabled"], binding.ssl_state)
    ])
    error_message = "Each app_service_custom_hostname_bindings ssl_state must be SniEnabled, IpBasedEnabled, or Disabled."
  }
}

variable "app_service_custom_hostname" {
  description = "Optional single custom hostname to bind to the App Service. Can be set with TF_VAR_app_service_custom_hostname from Bash, PowerShell, or Azure DevOps pipeline variables."
  type        = string
  default     = ""
}

variable "app_service_custom_hostname_ssl_state" {
  description = "SSL state for app_service_custom_hostname."
  type        = string
  default     = "SniEnabled"

  validation {
    condition     = contains(["SniEnabled", "IpBasedEnabled", "Disabled"], var.app_service_custom_hostname_ssl_state)
    error_message = "app_service_custom_hostname_ssl_state must be SniEnabled, IpBasedEnabled, or Disabled."
  }
}
