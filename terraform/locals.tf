...
locals {
  ...
  app_service_custom_hostname_bindings = merge(
    var.app_service_custom_hostname_bindings,
    trimspace(var.app_service_custom_hostname) != "" ? {
      primary = {
        hostname  = trimspace(var.app_service_custom_hostname)
        ssl_state = var.app_service_custom_hostname_ssl_state
      }
    } : {}
  )
