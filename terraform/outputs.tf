output "resource_group_name" {
  description = "Resource group used for the application resources."
...
output "app_service_custom_domain_cname_target" {
  description = "DNS CNAME target for custom domains that bind to the Linux App Service."
  value       = var.enable_app_service_stack ? module.app_service[0].default_hostname : null
}

output "app_service_custom_domain_verification_id" {
  description = "App Service custom domain verification ID for DNS TXT ownership validation."
  value       = var.enable_app_service_stack ? module.app_service[0].custom_domain_verification_id : null
  sensitive   = true
}
...
output "rag_storage_container_name" {
  description = "Document container name used for RAG source documents."
  value       = var.rag_storage_container_name
}

output "rag_openai_chat_deployment_name" {
  description = "Azure OpenAI chat deployment name exposed to the app."
  value       = local.rag_chat_deployment_name
}

output "rag_openai_embedding_deployment_name" {
  description = "Azure OpenAI embedding deployment name exposed to the app."
  value       = local.rag_embed_deployment_name
}

output "rag_search_index_name" {
  description = "Azure AI Search index name exposed to the app."
  value       = local.rag_search_index_name
}
