...
module "app_service_plan" {
  count  = local.feature_flags.enable_app_service_stack ? 1 : 0
...
module "app_registration_appservice" {
  count  = local.app_registration_enabled ? 1 : 0
...
module "app_service" {
  ...
  active_directory_client_id                    = var.app_service_enable_auth && local.app_registration_enabled ? module.app_registration_appservice[0].application_id : null
  log_analytics_workspace_id                    = local.log_analytics_workspace_id
  ...
  custom_hostname_bindings                      = local.app_service_custom_hostname_bindings

  app_settings = merge(
    local.app_service_app_settings,
    local.rag_search_query_key != "" ? {
      AZURE_SEARCH_QUERY_KEY = local.rag_search_query_key
    } : {},
    {
...
resource "azapi_resource_action" "rag_search_query_key" {
  count = local.feature_flags.create_azure_search_query_key && var.azure_search_query_key == "" && local.landingzone_azure_ai_search_lookup_enabled && length(data.azurerm_search_service.azure_ai_search) > 0 ? 1 : 0

  type        = "Microsoft.Search/searchServices@2024-06-01-preview"
  resource_id = data.azurerm_search_service.azure_ai_search[0].id
  action      = "createQueryKey/qk-${local.workload_slug}-${local.effective_environment}"
  method      = "POST"

  sensitive_response_export_values = ["key"]
}
...
resource "azurerm_role_assignment" "app_openai_user" {
  count = local.feature_flags.enable_app_service_stack && local.landingzone_openai_lookup_enabled && length(data.azurerm_cognitive_account.openai) > 0 ? 1 : 0
...
resource "azurerm_role_assignment" "app_search_index_data_reader" {
  count = local.feature_flags.enable_app_service_stack && local.landingzone_azure_ai_search_lookup_enabled && length(data.azurerm_search_service.azure_ai_search) > 0 ? 1 : 0
...
resource "azurerm_role_assignment" "app_ai_service_user" {
  count = local.feature_flags.enable_app_service_stack && local.landingzone_azure_ai_service_lookup_enabled && length(data.azurerm_cognitive_account.azure_ai_service) > 0 ? 1 : 0
