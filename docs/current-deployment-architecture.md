...
- For local Terraform runs, set `TF_VAR_app_service_custom_hostname` in Bash or PowerShell. In Azure DevOps, set the regular pipeline variable `APP_SERVICE_CUSTOM_HOSTNAME`; the pipeline exports the Terraform-compatible variable before plan/apply.
