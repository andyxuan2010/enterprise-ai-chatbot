...
6. The API queries Azure AI Search index `enterprise-docs` with hybrid text + vector search.
...
10. The API returns the answer and source paths to the browser.
...
| `AZURE_SEARCH_QUERY_KEY` | Optional, only set when `azure_search_query_key` is supplied |
...
- Query key is sensitive and must not be committed to tfvars.
...
- Add semantic ranking if the Search SKU supports it.
- Add reranking for top retrieved chunks.
- Add answer-level citation formatting with clickable source references.
