...
The ingestion script scans `DOCS_PATH` recursively and currently supports `.txt`, `.md`, and `.pdf`. Document IDs are deterministic and prefixed with a safe letter because Azure AI Search document keys cannot start with an underscore. Upload batch size defaults to `25` chunks and can be changed with `INGEST_UPLOAD_BATCH_SIZE`.
