...
12. The API builds context from the reranked chunks.
13. The API sends the original question, standalone retrieval question, retrieval queries, and reranked context to Azure OpenAI deployment `chat`.
14. The API returns the answer and retrieved source passages to the browser.
15. The browser renders clickable source buttons. When a source is selected, the right-side Source Preview panel shows metadata, the retrieved passage, and highlighted matching question terms.
