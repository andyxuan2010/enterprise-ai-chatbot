...
The package build stage uses `ADO_PACKAGE_PYTHON_VERSION` only for local packaging on the Azure DevOps runner. The App Service Python runtime remains controlled by `app_service_python_version` in each environment tfvars file.
