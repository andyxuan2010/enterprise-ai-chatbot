...
The current sandbox configuration in `terraform/environments/sandbox/terraform.tfvars` resolves to:
...
This workload reads those resources through Terraform data sources in `terraform/data.tf`.
...
- Optional private endpoint and private DNS patterns are preserved as commented code in `terraform/main.tf`.
- Optional custom hostname bindings are supported through `app_service_custom_hostname` for a single environment-provided hostname or `app_service_custom_hostname_bindings` for advanced multiple bindings. Public DNS must provide a CNAME from the custom hostname to the App Service default hostname before the App Service binding and managed certificate can be created.
- For local Terraform runs, set `TF_VAR_app_service_custom_hostname` in Bash or PowerShell. In Azure DevOps, set the regular pipeline/repo variable `app_service_custom_hostname`; the pipeline exports the Terraform-compatible variable before plan/apply.
...
The Azure DevOps pipeline uses `templateRepo` with `name: template`, so the shared template repository is resolved from the current Azure DevOps project. The checkout path remains `s/template`, preserving Terraform module paths that reference `../../template//modules/...`.

Shared/self-hosted runner hygiene is handled by `templates/shared-runner-hygiene.yml`. It creates job-scoped home/cache paths and removes stale Git `extraheader` credentials before and after checkout-heavy jobs.

The package build stage uses `ADO_PACKAGE_PYTHON_VERSION` from the `iac-shared-vars` variable group only for local packaging on the Azure DevOps runner. The App Service Python runtime remains controlled by `app_service_python_version` in each environment tfvars file.

Terraform Apply is gated by the regular Azure DevOps pipeline variable `ENABLE_ADO_TERRAFORM_APPLY=true`. App Service package deployment is gated separately by the regular pipeline variable `ENABLE_ADO_APP_DEPLOY=true`.

App Service package deployment uses async Azure CLI zip deployment so Kudu/Oryx can continue the remote build after the artifact is uploaded. This avoids failing the pipeline on Kudu gateway timeouts during longer sandbox builds.
