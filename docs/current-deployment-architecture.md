...
8. The API applies optional metadata filters such as source path, document title, section heading, product/service, date, version, URL, and access group.
9. The API reranks the hybrid candidates with the Azure OpenAI chat deployment when reranking is enabled.
10. The API builds context from the reranked chunks.
11. The API sends the user question and reranked context to Azure OpenAI deployment `chat`.
12. The API returns the answer and retrieved source passages to the browser.
13. The browser renders clickable source buttons. When a source is selected, the right-side Source Preview panel shows metadata, the retrieved passage, and highlighted matching question terms.
