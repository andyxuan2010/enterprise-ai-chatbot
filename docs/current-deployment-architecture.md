...
8. The API reranks the hybrid candidates with the Azure OpenAI chat deployment when reranking is enabled.
9. The API builds context from the reranked chunks.
10. The API sends the user question and reranked context to Azure OpenAI deployment `chat`.
11. The API returns the answer and retrieved source passages to the browser.
12. The browser renders clickable source buttons. When a source is selected, the right-side Source Preview panel shows the retrieved passage and highlights matching question terms.
