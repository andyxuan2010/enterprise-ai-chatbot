...
| `app_service_custom_hostname` | Optional single custom hostname to bind to App Service. Set with `TF_VAR_app_service_custom_hostname` in Bash, PowerShell, or Azure DevOps. |
| `app_service_custom_hostname_bindings` | Optional advanced map of custom hostnames to bind to App Service. Create public DNS CNAME records to the App Service default hostname before enabling. |
...
The Azure DevOps package build uses `ADO_PACKAGE_PYTHON_VERSION` from the `iac-shared-vars` variable group for the agent-side `UsePythonVersion@0` task. This is intentionally separate from Terraform's `app_service_python_version`, which controls the App Service runtime.

Set `ENABLE_ADO_TERRAFORM_APPLY=true` as a regular Azure DevOps pipeline variable to allow the Terraform Apply stage. Set `ENABLE_ADO_APP_DEPLOY=true` as a separate regular pipeline variable to allow the App Service package deployment stage.

For custom domains, set the single-hostname Terraform variable through the environment instead of committing hostnames in tfvars:

```bash
export TF_VAR_app_service_custom_hostname="chat-sbx.example.com"
```

```powershell
$env:TF_VAR_app_service_custom_hostname = "chat-sbx.example.com"
```

In Azure DevOps, set this as a regular pipeline/repo variable:

```text
app_service_custom_hostname = chat-sbx.example.com
```

The pipeline passes `app_service_custom_hostname` into Terraform's `TF_VAR_app_service_custom_hostname` before plan/apply. Optionally set `app_service_custom_hostname_ssl_state`; it defaults to `SniEnabled`.
